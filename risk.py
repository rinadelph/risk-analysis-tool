...
try:
    ...
    if not response:
        ...
        raise ConnectionError("Failed to get response from OpenAI API")
        
    logger.debug("OpenAI client initialized and tested successfully")
    
except Exception as e:
    logger.error(f"Failed to initialize OpenAI client: {e}")
    console.print("[red]Error: Failed to initialize OpenAI client. Please check your API key and connection.[/red]")
    console.print(f"[red]Details: {str(e)}[/red]")
    exit(1)  # Exit if we can't initialize the client
...
def download_10k_filings(ticker):
    ...
    try:
        ...
        for link in filing_links:
            ...
            download_filing(filing_url, ticker, raw_dir, headers)
...
def download_filing(filing_url, ticker, raw_dir, headers):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Save file
            output_file = raw_dir / f"{ticker}_10K_{formatted_date}.txt"
            with open(output_file, 'w', encoding='utf-8') as f:
                f.write(response.text)
            console.print(f"[green]Downloaded filing to: {output_file}[/green]")
            
            # Add delay to comply with SEC rate limits
            sleep(0.1)  # 100ms delay between requests
            
        else:
            console.print(f"[red]Failed to download filing: {response.status_code}[/red]")
...
def run_sp500_analysis():
    ...
    try:
            ...
            for idx, ticker in enumerate(tickers, 1):
                ...
                try:
                    ...
                    if len(risk_files) >= 2:
                        ...
                        error_msg = "Insufficient risk files available"
        ...
        console.print(f"Total Companies Processed: {len(tickers)}")
